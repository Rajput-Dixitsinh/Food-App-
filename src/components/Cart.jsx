...
const Cart = () => {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {cartItems?.length == 0 && (
        <div className="mt-4 flex flex-col items-center mb-24">
          <img className="h-44 mt-14" src="https://cdn-icons-png.flaticon.com/512/4555/4555971.png" alt="Empty Cart" />
          <h1 className="font-bold text-2xl mt-14">Cart Empty</h1>
          <h2 className="mt-2 font-semibold text-2xl">
            You can go to{" "}
            <a href="/" className="font-bold text-orange-600">
      ...
      {Object.values(uniqueFoodItems).map((item, index) => {
        ...
        const itemQty = item[1];
        return (
          <div className="border-b-4 w-full sm:w-[55%]" key={index}>
            <div className="flex my-4 items-center justify-between py-2 border-b-2">
      ...
      {cartItems.length > 0 && (
        <>
          <div className="flex flex-col w-full sm:w-[55%] my-4 border-b-2 border-b-black">
          ...
          <div className="w-full sm:w-[55%] mb-6">
            <div className="flex flex-col sm:flex-row justify-between font-bold">
              <div className="flex w-full sm:w-[15%] justify-between">
                <h2>TO PAY</h2>
                <span>₹{finalPrice}</span>
              </div>
              <div className="mt-2 sm:mt-0">
                <button
                  className="bg-orange-600 text-white mx-1 px-2"
                  onClick={() => handleClearCart()}
                >
                  Clear Cart
                </button>
                <button
                  className="bg-green-700 text-white mx-1 px-2"
                  onClick={() => {
                    handlePlaceOrder();
                  }}
                >
