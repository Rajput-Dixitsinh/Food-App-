...
//                       <button
//                         className="px-2 bg-orange-300 rounded mt-2 bg-white text-sm text-black font-bold border border-gray-500"
//                         onClick={() => {
//                           handleAddItem(info);
//                         }}
//                       >
...
//                       </button>
//                     ) : (
//                       <div className="flex items-center px-2 bg-orange-300 rounded mt-2 text-sm text-black font-bold border border-gray-500">
//                         <button
...
const RestaurantDetails = () => {
  ...
  return Object.keys(restaurantInfo || restaurantMenu).length === 0 ? (
    ...
    <div className="flex flex-col items-center">
      <div className="flex flex-col sm:flex-row justify-around w-full sm:w-3/5 border-b-2">
        ...
        <div className="w-full sm:w-1/2">
          <img
            className="h-24 sm:h-36 md:h-44"
            alt="restaurant-img"
            src={IMG_CDN_URL + cloudinaryImageId}
          />
          <div className="flex items-center">
      ...
      <div className="flex justify-around w-full sm:w-3/5 border-b-2 py-4">
        <div className="flex items-center">
          <p className="text-base font-bold">{costForTwoMessage}</p>
        </div>
        <div className="flex items-center">
      ...
      {restaurantMenu?.map((item) => {
        return (
          <div className="w-full sm:w-3/5 my-4 border-b-4" key={item?.card?.info?.id}>
            <h2 className="text-xl font-bold">{item?.card?.card?.title}</h2>
            {item?.card?.card?.itemCards?.map((list) => {
              const info = list?.card?.info;
              const price = String(info?.price);
              const slicedPrice = (price / 100).toFixed(2);
              return (
                ...
                >
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    {cartItems.filter((f) => f.id === info.id).length === 0 ? (
                      <button
                        className="px-2 bg-orange-300 rounded mt-2 text-sm text-black font-bold border border-gray-500"
                        onClick={() => {
                          handleAddItem(info);
                        }}
                      >
